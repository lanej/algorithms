...
func TestMinimumBribes(t *testing.T) {
	for x, i := range cases {
		fmt.Printf("Test: %d\n", x)
		answer := minimumBribes(i.input)

		if bribes, ok := answer.(int32); ok {
			if !reflect.DeepEqual(i.output, bribes) {
				t.Errorf("x: %d, Expected: %v, Actual: %v\n", x, i.output, bribes)
			}
		} else if message, ok := answer.(string); ok {
			if !reflect.DeepEqual(i.output, message) {
				t.Errorf("x: %d, Expected: %v, Actual: %v\n", x, i.output, answer)
			}
		} else {
